...
pub fn until(
  ...
  fn(parsed) { Ok(parsed) |> do_until(parser, terminator, None) }
}
...
pub fn until1(
  ...
  fn(parsed) { Ok(parsed) |> do_until(parser, terminator, Some(1)) }
}
...
pub fn skip_until1(terminator: Parser(a, d, c)) -> Parser(a, a, c) {
  until1(any(drop), terminator)
}
